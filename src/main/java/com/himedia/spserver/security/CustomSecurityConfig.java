...
@RequiredArgsConstructor // @Autowired 보다 더 강력한 자동 주입 어노테이션
@Log4j2 // security 에서 제공해주는 log 출력기능을 사용하겠습니다.
public class CustomSecurityConfig {

    @Bean
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        // security 시스템이 발동 후 가장 먼저 찾아 실행하는 함수(메서드)(Bean)
        ...
        //CSRF : 리퀘스트 위조방지 설정
        // CSRF 의 취약점은 공격자가 "사용자가 의도하지 않는 요청"을 수행하게 하는 취약점입니다.
        ...
        http.formLogin(config ->{
            config.loginPage("/members/loginLocal"); // loadUserByUsername 자동호출
            config.successHandler(new APILoginSuccessHandler()); // 로그인 성공시 실행할 코드를 갖는 클래스
            config.failureHandler(new APILoginFailHandler()); // 로그인 실패시 실행할 코드를 갖는 클래스
        });

        // JWT 액세스 토큰 체크
        http.addFilterBefore(new JWTCheckFilter(), UsernamePasswordAuthenticationFilter.class);

        // 접근시 발생한 예외 처리(액세스 토큰 오류, 로그인 오류 등등)
